...
class MultiBlockRBF(Kernel):
    """A block-wise RBF kernel with separate length scales per block."""

    def __init__(self, block_sizes, length_scales=None, length_scale_bounds=(1e-5, 1e5)):
    ...
    def __call__(self, X, Y=None, eval_gradient=False):
        ...
        if eval_gradient:
            for b in range(self.n_blocks):
                ...
                Yb = Y[:, start:end]
                dists = np.sum((Xb[:, np.newaxis, :] - Yb[np.newaxis, :, :]) ** 2, axis=2)
                grad_b = K * dists / (self.length_scales[b] ** 2)
    ...
    def __repr__(self):
        return f"MultiBlockRBF(block_sizes={self.block_sizes}, length_scales={self.length_scales})"
...
class Adiabat(Surrogate):
    ...
    def __init__(self, descriptor, kernel='RBF', nrestart=50,
        ...
        if kernel == 'RBF':
            ...
            self.kernel = C(hparam[0])*MultiBlockRBF(block_sizes=blocks,
                         length_scales=hparam[1:],
                         length_scale_bounds=(1e0, 1e5))
