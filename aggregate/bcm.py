...
class BCM():
    ...
    def __init__(self, surrogate):
        ...
        # after an expert is created, extended, replaced, or repartitioned.
        self._model_revision = 0

    @property
    def model_revision(self):
        return int(getattr(self, '_model_revision', 0))

    def _touch_model(self):
        self._model_revision = self.model_revision + 1

    #
    def _validate_pointwise_normalization(self):
        """Reject mixed globally-normalized experts before batch reduction.

        Ordinary BCM experts may legitimately use independently optimized
        kernels, so the strict checks activate only when global normalization
        metadata is present.  Workflow subclasses may provide a more detailed
        ``validate_global_normalization`` method; it is always run first.
        """
        ...
        reference = self.surrogates[0]
        if not hasattr(reference, 'normalization_id'):
            return

        attributes = (
            'normalization_id', 'normalization_version',
            'normalization_convention', 'alpha_scaled')
        factors = np.asarray(reference.target_unit_factors, dtype=float)
        scaler = reference.target_scaler
        kernels = np.asarray([
            model.kernel_.theta for model in reference.models], dtype=float)
        for expert_index, expert in enumerate(self.surrogates):
            for attribute in attributes:
                if getattr(expert, attribute, None) != getattr(
                        reference, attribute, None):
                    raise RuntimeError(
                        f'Expert {expert_index} has mixed {attribute}')
            if expert.target_scaler is not scaler:
                raise RuntimeError(
                    f'Expert {expert_index} does not share the target scaler')
            if not np.array_equal(
                    np.asarray(expert.target_unit_factors, dtype=float),
                    factors):
                raise RuntimeError(
                    f'Expert {expert_index} has mixed target-unit factors')
            expert_kernels = np.asarray([
                model.kernel_.theta for model in expert.models], dtype=float)
            if not np.allclose(
                    expert_kernels, kernels, rtol=0., atol=1.e-12):
                raise RuntimeError(
                    f'Expert {expert_index} does not use shared kernels')
            for target, model in enumerate(expert.models):
                if bool(model.normalize_y):
                    raise RuntimeError(
                        f'Expert {expert_index}, target {target} uses '
                        'normalize_y=True')
                if getattr(model, '_global_normalization_id', None) != \
                        reference.normalization_id:
                    raise RuntimeError(
                        f'Expert {expert_index}, target {target} has mixed '
    ...
    def _rebuild(self, surrogates, n_experts, enforce_size=False):
        ...
        for k in range(M):
            ...
            fit_model_data = getattr(new, 'fit_model_data', None)
            if fit_model_data is not None:
                # A globally-normalized surrogate stores PHYSICAL model
                # targets but must apply its shared transform at the GP
                # boundary.  Let it own that conversion during a repartition.
                fit_model_data(all_desc[idx], all_tgt[:, idx], hp_init)
            else:
                new.set_model_data(all_desc[idx], all_tgt[:, idx])
                for m in range(nmod):
                    # fit on the local arrays (representation-agnostic: CP
                    # stores targets, Adiabat stores energies/training)
    ...
    def evaluate_pointwise(self, gms, states=None, std=False):
        """Evaluate independent BCM marginals for one or many geometries.

        Unlike :meth:`evaluate`, this method never constructs an
        inter-geometry covariance matrix.  Every expert/target GP is called
        once with the complete geometry batch, then the scalar BCM precision
        correction is applied independently at every geometry.  Consequently
        a prediction cannot depend on which other trajectories happen to be
        alive in the same propagation batch.

        Aggregation is performed in the experts' raw model space.  This is
        important for globally-normalized CP models: their normalized
        coefficients are combined first and the expert reconstruction hook
        performs the single physical-unit inverse transform afterwards.
        """
        self._validate_pointwise_normalization()
        ...
        Xq, (ngm, _), singleX = utils.verify_geoms(gms)
        descriptor = self.surrogates[0].descriptor
        d_data = descriptor.generate(Xq)
        M = len(self.surrogates)
        nmod = self.surrogates[0].n_models()

        # (expert, target, geometry): each target GP sees the entire batch.
        means = np.zeros((M, nmod, ngm), dtype=float)
        ...
        tiny = 1.e-32
        precision = 1./np.maximum(variances, tiny)
        precision_sum = np.sum(precision, axis=0)
        weighted_mean = np.sum(precision*means, axis=0)

        aggregate_mean = np.zeros((nmod, ngm), dtype=float)
        aggregate_variance = np.zeros((nmod, ngm), dtype=float)
        for target, model in enumerate(self.surrogates[0].models):
            # kernel_.diag is the normalized-space prior variance when
            # normalize_y=False.  The y scale/mean retain compatibility with
            # ordinary sklearn-normalized experts.
            prior_variance = np.maximum(
                model.kernel_.diag(d_data)*model._y_train_std**2, tiny)
            prior_precision = 1./prior_variance
            prior_mean = float(np.ravel(model._y_train_mean)[0])
            aggregate_precision = (
                precision_sum[target] - (M - 1)*prior_precision)
            valid = aggregate_precision > tiny
            aggregate_variance[target, valid] = (
                1./aggregate_precision[valid])
            aggregate_mean[target, valid] = (
                aggregate_variance[target, valid]
                * (weighted_mean[target, valid]
                   - (M - 1)*prior_precision[valid]*prior_mean))

        if hasattr(self.surrogates[0], 'fold_baseline_mean'):
            self.surrogates[0].fold_baseline_mean(aggregate_mean, Xq)

        # A diagonal raw covariance contains exactly the pointwise marginals
        # required by the reconstruction hook, without any cross-trajectory
        # covariance.  The hook is called once after normalized aggregation.
        raw_covariance = np.zeros((nmod, ngm, ngm), dtype=float)
        diag = np.arange(ngm)
        raw_covariance[:, diag, diag] = aggregate_variance
        energy, energy_std, _ = self.surrogates[0].reconstruct_energy(
            aggregate_mean, raw_covariance, sts, std, False)
    ...
    #
    def _gradient_pointwise(self, gms, states=None, std=False):
        """Fast frozen-weight gradient path with pointwise BCM marginals."""
        self._validate_pointwise_normalization()
        ...
        descriptor = self.surrogates[0].descriptor
        d_data = descriptor.generate(Xq)
        d_gradient = descriptor.descriptor_gradient(Xq)
        M = len(self.surrogates)
        nmod = self.surrogates[0].n_models()

        means = np.zeros((M, nmod, ngm), dtype=float)
        variances = np.zeros_like(means)
        gradients = np.zeros((M, nmod, ngm, nc), dtype=float)
        for expert_index, expert in enumerate(self.surrogates):
            mean, pred_std, grad, _ = expert.raw_predict_and_grad(
                Xq, descrip=d_data, grad_descrip=d_gradient,
                std=True, cov=False)
            means[expert_index] = mean
            variances[expert_index] = pred_std**2
            gradients[expert_index] = grad
        ...
        weighted_gradient = np.sum(
            precision[..., None]*gradients, axis=0)

        aggregate_mean = np.zeros((nmod, ngm), dtype=float)
        aggregate_gradient = np.zeros((nmod, ngm, nc), dtype=float)
        aggregate_variance = np.zeros((nmod, ngm), dtype=float)
        for target, model in enumerate(self.surrogates[0].models):
            prior_variance = np.maximum(
                model.kernel_.diag(d_data)*model._y_train_std**2, tiny)
            ...
            aggregate_mean[target, valid] = (
                ...
                * (weighted_mean[target, valid]
                   - (M - 1)*prior_precision[valid]*prior_mean))
            # frozen_wts=True: differentiate expert means while holding all
            # pointwise precision weights fixed.
            aggregate_gradient[target, valid] = (
                aggregate_variance[target, valid, None]
                * weighted_gradient[target, valid])

        if hasattr(self.surrogates[0], 'fold_baseline_grad'):
            self.surrogates[0].fold_baseline_grad(aggregate_gradient, Xq)
        if hasattr(self.surrogates[0], 'fold_baseline_mean'):
            # The coefficient mean determines CP's root-map Jacobian.  This is
            # immaterial for an omega-only baseline, but essential for a
            # coefficient reference and for globally normalized targets.
            self.surrogates[0].fold_baseline_mean(aggregate_mean, Xq)

        result_gradient = np.zeros((len(sts), ngm, nc), dtype=float)
        result_std = np.zeros_like(result_gradient)
        zero_gcov = np.zeros((nmod, nc, nc), dtype=float)
        # CP/Adiabat reconstruction hooks currently accept one geometry.  All
        # expensive descriptor and GP work above remains fully batched.
        for geometry in range(ngm):
            grad, grad_std, _ = self.surrogates[0].reconstruct_gradient(
                aggregate_mean[:, geometry],
                aggregate_gradient[:, geometry], zero_gcov,
                sts, std, False)
            result_gradient[:, geometry] = grad
            if std:
                result_std[:, geometry] = grad_std

        if singleX:
            return utils.collect_output(
                (result_gradient[:, 0], result_std[:, 0]), (True, std))
        return utils.collect_output(
            (result_gradient, result_std), (True, std))

    #
    @timer.timed
    def gradient_pointwise(self, gms, states=None, std=False):
        """Public pointwise gradient API for batched propagation.

        The optimized pointwise contraction is available for frozen weights.
        For the conservative full-weight derivative, route through the general
        single-geometry BCM gradient implementation.
        """
        if self.frozen_wts:
            return self._gradient_pointwise(gms, states=states, std=std)
    ...
    def gradient(self, gms, states=None, std=False, cov=False,
        ...
        # Production dynamics requests only force means.  This path calls
        # every expert once for the complete geometry batch and avoids the
        # legacy geometry outer loop and gradient-covariance construction.
        if self.frozen_wts and not std and not cov:
            return self._gradient_pointwise(gms, states=states, std=False)
        ...
        for i in range(ngm):
            ...
            # Coefficient references alter the root-map Jacobian, so the
            # corresponding deterministic mean shift must also be restored
            # before gradient reconstruction.  An omega-only baseline leaves
            # the root Jacobian unchanged but is harmless here.
            self.surrogates[0].fold_baseline_mean(coeff_mean, Xq[i])
...
#    def gradient_pointwise(self, gms, states=None, std=False):
#        """Public pointwise gradient API for batched propagation."""
#        if not self.frozen_wts:
#            raise ValueError(
#                'gradient_pointwise requires frozen_wts=True')
#        return self._gradient_pointwise(gms, states=states, std=std)
#
#    #
#    #
